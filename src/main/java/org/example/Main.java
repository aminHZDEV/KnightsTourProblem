...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[][] move = new int[2][(width*width)+2];

        for(int i = 0 ;i < 2;i++)
            for (int j =0; j < width*width; j++)
                move[i][j] = -1;

        int [] startPoint = new int[]{4, 0};
        int[][] action = {
                {2,1},
                {2,-1},
                {-2,-1},
        ...
        if(!run(startPoint, action, environment, move, offset)){
            ...
            Thread visualizeThread = new Thread(() -> visualize(move));
    ...
    */
    ...
    /*
     add below visualization by take a little hint form theb.ai :)
    */

    private static void visualize(int[][] moves) {
